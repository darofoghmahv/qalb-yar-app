...
import { ChevronRight, ChevronLeft, HeartPulse, Wind, User, Stethoscope, Pill, Clipboard, AlertTriangle, Activity, FileText, X, BrainCircuit, Calculator, Info, BookUser, ClipboardList } from 'lucide-react';
...
const clinicalInfo = {
  heartSounds: { title: 'راهنمای سمع صداهای قلبی', content: ( <div className="space-y-4 text-right"> <p><strong>S1:</strong> صدای اول قلب، ناشی از بسته شدن دریچه‌های میترال و تریکوسپید.</p> <p><strong>S2:</strong> صدای دوم قلب، ناشی از بسته شدن دریچه‌های آئورت و پولمونر.</p> <p><strong>S3 (گالوپ بطنی):</strong> در نارسایی قلبی و افزایش حجم بطن شنیده می‌شود.</p> <p><strong>S4 (گالوپ دهلیزی):</strong> ناشی از انقباض دهلیز در مقابل یک بطن سفت (مثلاً در هیپرتروفی).</p> </div> ) },
  lungSounds: { title: 'راهنمای سمع صداهای ریوی', content: ( <div className="space-y-4 text-right"> <p><strong>کراکل یا رال (Crackles/Rales):</strong> صداهای کوتاه و منقطع. در ادم ریوی (نارسایی قلبی) شنیده می‌شود.</p> <p><strong>ویز (Wheezes):</strong> صداهای ممتد و موسیقایی. مشخصه آسم و COPD است.</p> </div> ) },
  ecgRhythms: { title: 'راهنمای ریتم‌های ECG', content: ( <div className="space-y-4 text-right"> <p><strong>ریتم سینوسی (Sinus Rhythm):</strong> ریتم طبیعی قلب. به ازای هر کمپلکس QRS یک موج P وجود دارد. ریت بین 60-100.</p> <p><strong>فیبریلاسیون دهلیزی (AFib):</strong> ریتم نامنظم بدون امواج P مشخص.</p> <p><strong>تاکی‌کاردی بطنی (VT):</strong> ریتم سریع با کمپلکس QRS پهن. یک آریتمی خطرناک.</p> </div> ) },
  prInterval: { title: 'فاصله PR', content: ( <div className="space-y-4 text-right"><p>نشان‌دهنده زمان هدایت از دهلیز به بطن است.</p><p><strong>محدوده نرمال:</strong> 120 تا 200 میلی‌ثانیه (3 تا 5 خانه کوچک).</p><p><strong>PR کوتاه ({'<'}120ms):</strong> می‌تواند نشانه سندرم‌های پیش‌تحریکی مانند WPW باشد.</p><p><strong>PR طولانی ({'>'}200ms):</strong> نشان‌دهنده بلوک دهلیزی-بطنی درجه اول (First-degree AV block) است.</p></div> ) },
  qrsDuration: { title: 'مدت زمان QRS', content: ( <div className="space-y-4 text-right"><p>نشان‌دهنده زمان دپولاریزاسیون بطن‌ها است.</p><p><strong>محدوده نرمال:</strong> کمتر از 120 میلی‌ثانیه ({'<'}3 خانه کوچک).</p><p><strong>QRS پهن ({'>='}120ms):</strong> می‌تواند به دلیل بلوک شاخه‌ای (BBB)، ریتم با منشا بطنی (مانند VT) یا هایپرکالمی باشد.</p></div> ) },
  qWaves: { title: 'امواج Q پاتولوژیک', content: ( <div className="space-y-4 text-right"><p>موج Q عمیق و پهن که می‌تواند نشانه سکته قلبی (MI) پیشین باشد.</p><p><strong>معیارها:</strong> عرض بیشتر از 40 میلی‌ثانیه ({'>'}1 خانه کوچک) یا عمق بیشتر از 25% ارتفاع موج R در همان لید.</p><p>وجود آن در لیدهای مرتبط با یک دیواره خاص قلب اهمیت دارد.</p></div> ) },
  morphologies: { title: 'مورفولوژی‌های خاص QRS', content: ( <div className="space-y-4 text-right"><p><strong>بلوک شاخه راست (RBBB):</strong> الگوی RSR' ("گوش خرگوشی") در لید V1 و موج S پهن در لیدهای V6 و I.</p><p><strong>بلوک شاخه چپ (LBBB):</strong> کمپلکس QS یا rS پهن در V1 و موج R پهن و مونوفازیک در V6.</p><p><strong>هیپرتروفی بطن چپ (LVH):</strong> ولتاژ بالای QRS. یکی از معیارها: (موج S در V1) + (موج R در V5 یا V6) {'>'} 35 میلی‌متر.</p></div> ) },
};
...
// --- Step Components (abbreviated where unchanged) ---
...
const ProgressBar = ({ currentStep, goToStep }) => {
    ...
    return (<div className="flex justify-between items-center">{steps.map((s, i) => (<React.Fragment key={s.number}><div className="flex flex-col items-center cursor-pointer" onClick={() => goToStep(s.number)}><div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center transition-all ${currentStep >= s.number ? 'bg-red-500 text-white' : 'bg-slate-200 text-slate-400'}`}>{React.cloneElement(s.icon, { className: 'w-5 h-5 md:w-6 md:h-6' })}</div><p className={`mt-2 text-xs text-center font-semibold ${currentStep >= s.number ? 'text-red-500' : 'text-slate-400'}`}>{s.label}</p></div>{i < steps.length - 1 && <div className={`flex-1 h-1 mx-1 md:mx-2 rounded-full ${currentStep > i + 1 ? 'bg-red-500' : 'bg-slate-200'}`}></div>}</React.Fragment>))}</div>);
};
...
const ChiefComplaint = ({ data, setData }) => { const { chiefComplaint, hpi } = data; const setChiefComplaint = (value) => setData(prev => ({ ...prev, chiefComplaint: value })); const updateHpi = (complaint, field, value) => setData(prev => ({ ...prev, hpi: { ...prev.hpi, [complaint]: { ...prev.hpi[complaint], [field]: value } } })); const handleSOBTypeChange = (type) => { const currentTypes = hpi.shortnessOfBreath.type || []; const newTypes = currentTypes.includes(type) ? currentTypes.filter(t => t !== type) : [...currentTypes, type]; updateHpi('shortnessOfBreath', 'type', newTypes); }; const complaintOptions = [ { id: 'chestPain', label: 'درد قفسه سینه', icon: <HeartPulse /> }, { id: 'shortnessOfBreath', label: 'تنگی نفس', icon: <Wind /> }, { id: 'palpitations', label: 'تپش قلب', icon: <Activity /> }, ]; return ( <div className="animate-fade-in"> <h2 className="text-2xl font-bold mb-6 text-slate-700 flex items-center gap-2"><AlertTriangle className="text-red-500"/>شکایت اصلی</h2> <div className="mb-6"> <p className="mb-3 font-semibold text-slate-600">شکایت اصلی بیمار را انتخاب کنید:</p> <div className="flex flex-wrap gap-3"> {complaintOptions.map(opt => ( <button key={opt.id} onClick={() => setChiefComplaint(opt.id)} className={`flex items-center gap-2 p-3 rounded-lg border-2 transition-all duration-200 ${chiefComplaint === opt.id ? 'bg-red-500 text-white border-red-500' : 'bg-white text-slate-700 border-slate-300 hover:border-red-400'}`}> {opt.icon} {opt.label} </button> ))} </div> </div> {chiefComplaint === 'chestPain' && <div className="p-4 bg-red-50 rounded-lg border border-red-200 animate-fade-in"><h3 className="text-lg font-bold mb-4 text-red-800">جزئیات درد قفسه سینه (OPQRST)</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><input type="text" value={hpi.chestPain.onset} onChange={e => updateHpi('chestPain', 'onset', e.target.value)} placeholder="شروع و زمان‌بندی (Onset)" className="p-2 border rounded-md" /><input type="text" value={hpi.chestPain.quality} onChange={e => updateHpi('chestPain', 'quality', e.target.value)} placeholder="کیفیت (Quality) - مثلا فشاری، تیز" className="p-2 border rounded-md" /><input type="text" value={hpi.chestPain.radiation} onChange={e => updateHpi('chestPain', 'radiation', e.target.value)} placeholder="انتشار (Radiation)" className="p-2 border rounded-md" /><div className="flex flex-col"><label className="text-sm text-slate-600 mb-1">شدت (Severity): {hpi.chestPain.severity}/10</label><input type="range" min="0" max="10" value={hpi.chestPain.severity} onChange={e => updateHpi('chestPain', 'severity', e.target.value)} className="w-full" /></div><input type="text" value={hpi.chestPain.provocative} onChange={e => updateHpi('chestPain', 'provocative', e.target.value)} placeholder="عوامل تشدید کننده" className="p-2 border rounded-md" /><input type="text" value={hpi.chestPain.palliative} onChange={e => updateHpi('chestPain', 'palliative', e.target.value)} placeholder="عوامل تسکین دهنده" className="p-2 border rounded-md" /></div></div>} {chiefComplaint === 'shortnessOfBreath' && <div className="p-4 bg-blue-50 rounded-lg border border-blue-200 animate-fade-in"><h3 className="text-lg font-bold mb-4 text-blue-800">جزئیات تنگی نفس</h3><div className="space-y-4"><input type="text" value={hpi.shortnessOfBreath.onset} onChange={e => updateHpi('shortnessOfBreath', 'onset', e.target.value)} placeholder="شروع و مدت زمان" className="p-2 border rounded-md w-full" /><div><p className="font-semibold mb-2">نوع تنگی نفس:</p><div className="flex flex-wrap gap-2">{['در حالت استراحت', 'هنگام فعالیت (DOE)', 'در حالت درازکش (Orthopnea)', 'حمله‌ای شبانه (PND)'].map(type => (<label key={type} className="flex items-center gap-2 p-2 border rounded-md cursor-pointer"><input type="checkbox" checked={(hpi.shortnessOfBreath.type || []).includes(type)} onChange={() => handleSOBTypeChange(type)} className="form-checkbox text-blue-500" />{type}</label>))}</div></div><input type="text" value={hpi.shortnessOfBreath.associated} onChange={e => updateHpi('shortnessOfBreath', 'associated', e.target.value)} placeholder="علائم همراه (مثلا سرفه، ورم پا)" className="p-2 border rounded-md w-full" /></div></div>} {chiefComplaint === 'palpitations' && <div className="p-4 bg-green-50 rounded-lg border border-green-200 animate-fade-in"><h3 className="text-lg font-bold mb-4 text-green-800">جزئیات تپش قلب</h3><div className="grid grid-cols-1 md:grid-cols-3 gap-4"><input type="text" value={hpi.palpitations.frequency} onChange={e => updateHpi('palpitations', 'frequency', e.target.value)} placeholder="تکرار (Frequency)" className="p-2 border rounded-md" /><input type="text" value={hpi.palpitations.duration} onChange={e => updateHpi('palpitations', 'duration', e.target.value)} placeholder="مدت هر بار (Duration)" className="p-2 border rounded-md" /><input type="text" value={hpi.palpitations.triggers} onChange={e => updateHpi('palpitations', 'triggers', e.target.value)} placeholder="عوامل شروع‌کننده (Triggers)" className="p-2 border rounded-md" /></div></div>} </div> ); };
...
const ECGFindings = ({ data: {ecg}, updateData, openModal }) => {
    const handleMultiCheckboxChange = (field, value) => {
        const currentValues = ecg[field] || [];
        const newValues = currentValues.includes(value) ? currentValues.filter(v => v !== value) : [...currentValues, value];
        updateData('ecg', field, newValues);
    };
    ...
    return (
        <div className="animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-6">
                <div>
                    <label className="font-semibold text-slate-600 flex items-center gap-2">ریتم <InfoButton onClick={() => openModal(clinicalInfo.ecgRhythms.title, clinicalInfo.ecgRhythms.content)}/></label>
                    <select value={ecg.rhythm} onChange={e => updateData('ecg', 'rhythm', e.target.value)} className="w-full p-2 mt-1 border rounded-md bg-white">
                        <option>سینوسی</option><option>فیبریلاسیون دهلیزی</option><option>فلوتر دهلیزی</option><option>SVT</option><option>VT</option>
                    </select>
                </div>
                <div>
                    <label className="font-semibold text-slate-600">ریت (Rate)</label>
                    <div className="flex items-center gap-2"><input type="number" value={ecg.rate} onChange={e => updateData('ecg', 'rate', e.target.value)} placeholder="bpm" className="w-full p-2 mt-1 border rounded-md" /><CalculatorButton text="" onClick={showRateCalculator} /></div>
                </div>
                <div>
                    <label className="font-semibold text-slate-600">محور (Axis)</label>
                    <div className="flex items-center gap-2"><select value={ecg.axis} onChange={e => updateData('ecg', 'axis', e.target.value)} className="w-full p-2 mt-1 border rounded-md bg-white"><option>نرمال</option><option>انحراف به چپ</option><option>انحراف به راست</option><option>نامشخص</option></select><CalculatorButton text="" onClick={showAxisCalculator} /></div>
                </div>
                <div>
                    <label className="font-semibold text-slate-600 flex items-center gap-2">فاصله PR <InfoButton onClick={() => openModal(clinicalInfo.prInterval.title, clinicalInfo.prInterval.content)} /></label>
                    <input type="number" value={ecg.pr} onChange={e => updateData('ecg', 'pr', e.target.value)} placeholder="ms" className="w-full p-2 mt-1 border rounded-md" />
                </div>
                <div>
                    <label className="font-semibold text-slate-600 flex items-center gap-2">مدت QRS <InfoButton onClick={() => openModal(clinicalInfo.qrsDuration.title, clinicalInfo.qrsDuration.content)} /></label>
                    <input type="number" value={ecg.qrs} onChange={e => updateData('ecg', 'qrs', e.target.value)} placeholder="ms" className="w-full p-2 mt-1 border rounded-md" />
                </div>
                <div>
                    <label className="font-semibold text-slate-600">فاصله QTc</label>
                    <div className="flex items-center gap-2"><input type="number" value={ecg.qtc} onChange={e => updateData('ecg', 'qtc', e.target.value)} placeholder="ms" className="w-full p-2 mt-1 border rounded-md" /><CalculatorButton text="" onClick={showQTcCalculator} /></div>
                </div>

                <div className="lg:col-span-3">
                    <label className="font-semibold text-slate-600">تغییرات قطعه ST</label>
                    <div className="flex flex-col sm:flex-row gap-4 mt-2 p-3 bg-slate-50 rounded-md">
                        <div className="flex-1 space-y-2">
                            <p className="font-medium text-sm">نوع تغییرات:</p>
                            {['ST Elevation', 'ST Depression', 'T Wave Inversion'].map(opt => (
                                <label key={opt} className="flex items-center gap-2"><input type="checkbox" checked={ecg.stChanges.includes(opt)} onChange={() => handleMultiCheckboxChange('stChanges', opt)} className="form-checkbox text-red-500" />{opt}</label>
                            ))}
                        </div>
                        <div className="flex-1 space-y-2">
                            <p className="font-medium text-sm">محل تغییرات:</p>
                            {leadGroups.map(opt => (
                                <label key={opt} className="flex items-center gap-2"><input type="checkbox" checked={ecg.stLocation.includes(opt)} onChange={() => handleMultiCheckboxChange('stLocation', opt)} className="form-checkbox text-red-500" />{opt}</label>
                            ))}
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-3">
                    <label className="font-semibold text-slate-600 flex items-center gap-2">مورفولوژی و یافته‌های دیگر <InfoButton onClick={() => openModal(clinicalInfo.morphologies.title, clinicalInfo.morphologies.content)} /></label>
                     <div className="flex flex-col sm:flex-row gap-4 mt-2 p-3 bg-slate-50 rounded-md">
                        <div className="flex-1 space-y-2">
                             <p className="font-medium text-sm">مورفولوژی‌های خاص:</p>
                            {morphologyOptions.map(opt => (
                                <label key={opt} className="flex items-center gap-2"><input type="checkbox" checked={ecg.morphologies.includes(opt)} onChange={() => handleMultiCheckboxChange('morphologies', opt)} className="form-checkbox text-red-500" />{opt}</label>
                            ))}
                        </div>
                        <div className="flex-1 space-y-2">
                            <label className="font-semibold text-slate-600 flex items-center gap-2">امواج Q پاتولوژیک <InfoButton onClick={() => openModal(clinicalInfo.qWaves.title, clinicalInfo.qWaves.content)} /></label>
                            <div className="flex flex-col gap-2">
                                <label className="flex items-center gap-2"><input type="checkbox" checked={ecg.hasQWaves} onChange={(e) => updateData('ecg', 'hasQWaves', e.target.checked)} className="form-checkbox text-red-500" />وجود دارد</label>
                                {ecg.hasQWaves && (
                                    <select multiple value={ecg.qWaveLocation} onChange={(e) => updateData('ecg', 'qWaveLocation', Array.from(e.target.selectedOptions, option => option.value))} className="w-full p-2 mt-1 border rounded-md bg-white text-sm">
                                        {leadGroups.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                    </select>
                                )}
                            </div>
                        </div>
                ...
                <div className="lg:col-span-3">
                    <label className="font-semibold text-slate-600">سایر توضیحات ECG</label>
                    <textarea value={ecg.other} onChange={e => updateData('ecg', 'other', e.target.value)} rows="3" placeholder="سایر یافته‌های مهم مانند آریتمی‌های دیگر، Poor R wave progression و ..." className="w-full p-2 mt-1 border rounded-md"></textarea>
                </div>
            </div>
        </div>
    );
...
const Summary = ({ data, goToStep }) => {
  ...
  const summaryText = useMemo(() => {
    ...
    const complaints = { chestPain: 'درد قفسه سینه', shortnessOfBreath: 'تنگی نفس', palpitations: 'تپش قلب' };
    ...
    text += hpiIntro;
    if (chiefComplaint === 'chestPain') { const cp = hpi.chestPain; text += `درد بیمار از ${cp.onset || '---'} شروع شده، کیفیت آن ${cp.quality || '---'} بوده و به ${cp.radiation || '---'} انتشار دارد. شدت درد ${cp.severity}/10 گزارش شده است. عوامل تشدید کننده شامل ${cp.provocative || '---'} و عوامل تسکین دهنده شامل ${cp.palliative || '---'} می‌باشند.\n\n`; }
    else if (chiefComplaint === 'shortnessOfBreath') { const sob = hpi.shortnessOfBreath; text += `تنگی نفس بیمار از ${sob.onset || '---'} شروع شده و از نوع ${sob.type.join(', ') || '---'} می‌باشد. علائم همراه شامل ${sob.associated || '---'} است.\n\n`; }
    else if (chiefComplaint === 'palpitations') { const palp = hpi.palpitations; text += `تپش قلب بیمار با تکرار ${palp.frequency || '---'} و مدت زمان ${palp.duration || '---'} در هر بار اتفاق می‌افتد. عوامل شروع کننده شامل ${palp.triggers || '---'} می‌باشد.\n\n`; }
    else { text += '\n\n'; }

    text += `** سوابق پزشکی، جراحی و دارویی **\n`;
    text += `- سابقه پزشکی (PMH): ${pastMedicalHistory || 'موردی ذکر نشده'}\n`;
    text += `- سابقه جراحی (PSH): ${pastSurgicalHistory || 'موردی ذکر نشده'}\n`;
    const activeMeds = medications.filter(med => med.name);
    const drugHistory = activeMeds.length > 0 ? activeMeds.map(m => `${m.name} (${m.dosage || 'دوز نامشخص'})`).join('، ') : 'دارویی مصرف نمی‌کند.';
    text += `- سابقه دارویی (DH): ${drugHistory}\n\n`;

    text += `** حساسیت‌ها (Allergies) **\n${allergies || 'موردی ذکر نشده'}\n\n`;

    text += `** مرور سیستم‌ها (ROS) **\n`;
    const positiveRos = Object.entries(ros).map(([systemKey, symptoms]) => {
        if (symptoms.length > 0) {
            return `- ${rosSystems[systemKey].label}: ${symptoms.join('، ')}`;
        }
        return null;
    }).filter(Boolean);
  ...
  return (
    <div className="animate-fade-in">
      <h2 className="text-2xl font-bold mb-6 text-slate-700 flex items-center gap-2"><Clipboard className="text-red-500"/>خلاصه و خروجی</h2>
      <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg whitespace-pre-wrap font-mono text-sm leading-relaxed">{summaryText}</div>
      <button onClick={copyToClipboard} className="mt-6 w-full bg-red-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-red-600 transition flex items-center justify-center gap-2">{copied ? 'کپی شد!' : 'کپی کردن خلاصه'}<Clipboard /></button>
      <div className="mt-8">
...
// --- DDx Logic ---
function generateDDx(data) { const ddx = []; const { chiefComplaint, hpi, riskFactors, physicalExam, ecg } = data; const hasRiskFactors = Object.values(riskFactors).some(v => v); if (chiefComplaint === 'chestPain') { if (ecg.stChanges.includes('ST Elevation')) { ddx.push({ name: 'سکته قلبی حاد با صعود قطعه ST (STEMI)', reason: 'به دلیل درد قفسه سینه به همراه ST Elevation در ECG.' }); } if (ecg.stChanges.includes('ST Depression') || ecg.stChanges.includes('T Wave Inversion')) { ddx.push({ name: 'سندروم کرونری حاد بدون صعود قطعه ST (NSTE-ACS)', reason: 'به دلیل درد قفسه سینه به همراه تغییرات ST/T در ECG.' }); } if (hpi.chestPain.quality.includes('تیز') || hpi.chestPain.quality.includes('پلورتیک')) { ddx.push({ name: 'پریکاردیت یا آمبولی ریه', reason: 'به دلیل ماهیت تیز و پلورتیک درد قفسه سینه.' }); } } if (chiefComplaint === 'shortnessOfBreath') { if (physicalExam.lungSounds.includes('کراکل') || physicalExam.jvp.includes('برجسته')) { ddx.push({ name: 'نارسایی احتقانی قلب (CHF)', reason: 'به دلیل تنگی نفس به همراه کراکل ریوی و JVP برجسته.' }); } } if (chiefComplaint === 'palpitations') { if (ecg.rhythm === 'فیبریلاسیون دهلیزی') { ddx.push({ name: 'فیبریلاسیون دهلیزی (Atrial Fibrillation)', reason: 'به دلیل شکایت تپش قلب و ریتم AFib در ECG.' }); } } if (ddx.length === 0 && hasRiskFactors && chiefComplaint === 'chestPain') { ddx.push({ name: 'آنژین پایدار یا ناپایدار', reason: 'به دلیل درد قفسه سینه در بیمار با عوامل خطر قلبی.' }); } return ddx; }
